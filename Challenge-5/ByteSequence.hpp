...
public:
  ...
  ByteSequence getRepeatingKeyXorEncryptedByteSequence(
    const ByteSequence& key)
  {
    std::vector<char> encryptedBytes(*_bytes.get());
    auto keyBytes = key.getBytes();

    for (auto i = 0; i < encryptedBytes.size(); ++i)
    {
  ...
  bool operator<(const ByteSequence& byteSequence)
  {
    return false;
  }
...
private:
    ...
    const std::vector<char>& bytes2)
  {
    ...
    for (auto i = 0; i < xoredBytes.size(); ++ i)
    {
