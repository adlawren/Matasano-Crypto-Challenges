...
private:
  ...
  static void shouldGetXoredBytes()
  {
    std::string s1 = "68656c6c6f";
    std::vector<char> hexEncodedAsciiBytes1(&s1[0], &s1[ s1.size() ]);
    std::string s2 = "1a6f54be3d";
    std::vector<char> hexEncodedAsciiBytes2(&s2[0], &s2[ s2.size() ]);
    std::string s3 = "720a38d252";
    std::vector<char> expectedAsciiBytes(&s3[0], &s3[ s3.size() ]);

    auto decodedBytes1 =
      ByteSequence::getDecodedHexEncodedAsciiBytes(hexEncodedAsciiBytes1);
    auto decodedBytes2 =
      ByteSequence::getDecodedHexEncodedAsciiBytes(hexEncodedAsciiBytes2);
    auto xoredBytes = ByteSequence::getXoredBytes(decodedBytes1, decodedBytes2);
    auto encodedXoredBytes =
      ByteSequence::getHexEncodedAsciiBytes(xoredBytes);

    assert(encodedXoredBytes == expectedAsciiBytes);
  }
};
